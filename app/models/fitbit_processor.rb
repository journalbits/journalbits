...
module ServiceProcessor
  class FitbitProcessor
    def initialize date, user
      @client = client = Fitgem::Client.new( { consumer_key: ENV['FITBIT_CONSUMER_KEY'], consumer_secret: ENV['FITBIT_CONSUMER_SECRET'], oauth_token: "default", oauth_secret: "default" } )
    ...
    def save_activity_entries_on_date
      ...
      unless FitbitActivityEntry.exists?(date: @date.to_s[0..9], user_id: @user.id)
        FitbitActivityEntry.create(calories: activity_data[:calories], distance: activity_data[:distance], steps: activity_data[:steps], active_minutes: activity_data[:active_minutes], date: @date.to_s[0..9], user_id: @user.id)
      end
    end
    ...
    def save_weight_entries_on_date
      ...
      unless FitbitWeightEntry.exists?(date: @date.to_s[0..9], user_id: @user.id)
        FitbitWeightEntry.create(weight: weight_data[:weight], weight_unit: weight_data[:weight_unit], date: @date.to_s[0..9], user_id: @user.id)
      end
    end
  end
end
